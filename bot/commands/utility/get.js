...
module.exports = {
    ...
    .setName('get')
        .setDescription('Buy the stock you want')
    ...
    async execute(interaction){
        ...
        if(messageUndone != 'Transaction Complete!')
                {
                    interaction.reply(`You do not have enough gold to complete the transaction`)
                    return
                }
                const embed = new EmbedBuilder()
                .setTitle(messageUndone)
                .setDescription(`${interaction.user.globalName}`)
                .setFields({name: `${quantity} stocks`, value: `${stock_id} brought for ${data.total} gp total`})
                .setColor(0x03fc20)
